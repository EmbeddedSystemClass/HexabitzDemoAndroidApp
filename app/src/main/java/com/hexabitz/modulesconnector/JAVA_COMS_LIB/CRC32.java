...
public class CRC32 {
	...
	//this is the academic abc CRC code.. it works :)
	public static int calc2(byte[] ba, int offset, int count)
	 {
	...
	public static int calc(byte[] ba, int offset, int count) {
		...
		crc = 0xffffffff;
		for (int j = offset; j < count + offset; j++) {
			crc = (crc << 8) ^ crctab[((crc >> 24) ^ ba[j]) & 0xFF];
		}
		return crc;
	}

	public static void makeTable() {
		int poly = 0x04C11DB7; // MPEG-2 CRC polynomial
		int crc;

		for (int i = 0; i < 256; i++) {
			crc = reverseByte(i) & 0xff;
			for (int j = 0; j < 8; j++) {
				if ((crc & 1) > 0)
					crc = (crc >>> 1) ^ poly;
				else
			...
			crctab[i] = reverseInt(crc);
		}
	}
	/*
	 * /* sectioncrc.c -- MPEG-2 checksum calculation, Python Module
	 * 
	 ...
	 * #include <stdint.h>
	 * 
	 * static uint32_t const crctab[256] = { 0x00000000, 0x04c11db7, 0x09823b6e,
	 ...
	 * 
	 * 
	 * uint32_t sectioncrc( unsigned char *buf, unsigned int len ) { unsigned
	 * char *p; uint32_t crc;
	 * 
	 * crc = 0xffffffff; p = buf; while( len-- ) { crc = (crc<<8) ^
	 * crctab[((crc>>24) ^ *(p++)) & 0xFF]; }
	 * 
	 * return crc; }
	 */
}
